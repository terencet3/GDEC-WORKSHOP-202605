...
In a Snowsight worksheet, create your instance:
...
Wait for the instance to reach READY. Poll with:
...
From your laptop terminal, connect with psql using the application user from step 1:

psql "host=<host> user=application password=<password> dbname=postgres sslmode=require"
Checkpoint A. At the psql prompt:

SELECT version();
You should see PostgreSQL 17.x.

Create a small transactional schema for cart and checkout:
...
Demonstrate transactional behavior. Open a second psql window connected to the same instance, then in window 1:
...
In your psql session against gdec_pg_<NN>, enable pg_lake:

CREATE EXTENSION IF NOT EXISTS pg_lake CASCADE;
Create an Iceberg table that mirrors paid checkouts joined with cart items:
...
WHERE  c.status = 'PAID';
Checkpoint A. In psql:

SELECT COUNT(*) FROM commerce.checkout_iceberg;
Note the count (should be roughly 140 rows, ±20).

Switch to Snowsight. Create a catalog integration that points to your Postgres instance:
...
CREATE OR REPLACE CATALOG INTEGRATION gdec_pg_cat_<NN>
  ...
  ENABLED = TRUE;
Create a Snowflake Iceberg table that references the Postgres-managed table:
...
Combine fresh transactional data with the warehouse PRODUCT_REVIEWS. For example, summarize recent reviews for products customers are buying right now:
...
LIMIT 5;
Checkpoint C. You should see 5 rows; each REVIEW_SUMMARY should be a short paragraph.

Cleanup (run in Snowsight as ACCOUNTADMIN):
